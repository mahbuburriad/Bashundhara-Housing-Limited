    ...
    {
      $email = mysqli_query($con, "SELECT * FROM email Where reciever LIKE '%security%' ORDER BY date_e DESC");
      mysqli_close($con);
    }
		?>
...
<form name="email" method="post" action="searchdateeaaa.php">
<strong>Date</strong>
  ...
  <form name="emailsu" method="post" action="searchesa.php">
  <strong>Email Subject</strong>
  <input type="text" name= "subject" placeholder="Enter Email Subject" style="width:200px; padding:5px; border-radius: 8px;" />

<input type="submit" name="submit" value="Go" style="width:40px; padding:3px; border-radius: 40px;" />

    <a style="width: 100px; padding: 3px; font-size: 1.2em; border-radius: 30px; cursor: pointer; background-color: silver;" href="totalmailsga.php">Total Mail By Date</a>
  </p>
</form>
