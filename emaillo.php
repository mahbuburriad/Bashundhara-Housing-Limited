    ...
    {
      $email = mysqli_query($con, "SELECT * FROM email Where reciever LIKE '%land_owner%' ORDER BY date_e DESC");
      mysqli_close($con);
    }
		?>
...
<form name="email" method="post" action="searchdateel.php">
<strong>Date</strong>
  ...
  <form name="emailsu" method="post" action="searchesl.php">
  <strong>Email Subject</strong>
  <input type="text" name= "subject" placeholder="Enter Email Subject" style="width:200px; padding:5px; border-radius: 8px;" />

<input type="submit" name="submit" value="Go" style="width:40px; padding:3px; border-radius: 40px;" />

    <a style="width: 100px; padding: 3px; font-size: 1.2em; border-radius: 30px; cursor: pointer; background-color: silver;" href="totalmailsg.php">Total Mail By Date</a>
  </p>
</form>
